...
class Web3Store {
  ...
  async getUserTokens({ trustApiName, defaultAccount }) {
    return new Promise((resolve, reject) => {
      ...
      }else if ("eramsmartchain" === blockchain_network) {
        api_domain = "eramscan.com";
        etherscanApiKey = process.env["REACT_APP_PROXY_MULTISENDER_ZEDXION_API_KEY"];
      } else {
      ...
      if ("eramsmartchain" === blockchain_network) {
        apiHref = `https://eramscan.com`;
      }
      ...
      window.fetch(
        ...
        if (('mumbai' === blockchain_network) || ('matic' === blockchain_network)) {
          ...
          tokensUnique.unshift({
            value: '0x000000000000000000000000000000000000bEEF',
            label: "ERAM - ERAM Native Currency"
          })
        }
