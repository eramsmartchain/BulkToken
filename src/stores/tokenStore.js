...
class TokenStore {
  ...
  async proxyMultiSenderAddress() {
    try {
      ...
      const netIdEnvVarName =
        ...
        console.log('meo1: ', netIdEnvVarName);
  ...
  @computed get addressesStringsData() {
    let addressesString="";
    let arrayString=[];
    this.jsonAddresses.map((account, index) => {
      if (((index + 1) % 100) == 0) {
        addressesString += Object.keys(account)[0];
        arrayString.push(addressesString);
        addressesString="";
      } else {
        addressesString += Object.keys(account)[0] + ',';
      }
    });
    arrayString.push(addressesString.slice(0, -1));
    return arrayString;
  }
